...
export default function FAQ({ dict }: FAQProps) {
  const faqs = dict?.faq?.questions && dict.faq.questions.length > 0
    ? dict.faq.questions
    : DEFAULT_FAQS;

  const faqTitle = dict?.faq?.title || 'Frequently Asked Questions';
  const faqSubtitle = dict?.faq?.subtitle || 'Everything you need to know about ordering, delivery, and catering in Newark.';
